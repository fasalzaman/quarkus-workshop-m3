...
In this step, we'll use a ConfigMap to handle this capability. Go to the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-codeready/configmaps[Config Maps^] in Developer Console, then click on `Create Config Map`:
...
----
mvn -q quarkus:add-extension -Dextensions="kubernetes-config" -Duser.home=/home/jboss -f $CHE_PROJECTS_ROOT/quarkus-workshop-m3-labs/quarkus-petclinic-web-v2
----
...
Let's modify the Qute template to show the message that refers to OpenShift ConfigMap. Open `welcome.html` file in `src/main/resources/templates` of *quarkus-petclinic-web-v2* project, and replace the following code at `h2` tag:
...
[source,sh,role="copypaste"]
----
mvn clean package -Duser.home=/home/jboss -f $CHE_PROJECTS_ROOT/quarkus-workshop-m3-labs/quarkus-petclinic-web-v2
----
...
Once the rollout is complete, back on the http://petclinic-web-v2-{{ USER_ID }}-codeready.{{ ROUTE_SUBDOMAIN}}[Welcome Page^] then you shoud see this:
...
[source,sh,role="copypaste"]
----
mvn clean package -Duser.home=/home/jboss -f $CHE_PROJECTS_ROOT/quarkus-workshop-m3-labs/quarkus-petclinic-customers-service && oc rollout status -w dc/customers-service
----
...
Back on the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-codeready[Topology View^] and click on `DC(customers-service)` then select `Edit Health Checks` in *Actions* dropbox:

image::M3-coustmers-health-check.PNG[health, 700]
