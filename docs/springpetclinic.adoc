...
### Switching Java language server to Standard mode

On the right side bottom end you will find a Rocket icon which shows that Java language server running in light-weight mode, Click on the rocket icon and select `Yes`  in the popup window to switch to Standard Mode.
...
[source,sh,role="copypaste"]
----
mvn spring-boot:run -Duser.home=/home/jboss -f $CHE_PROJECTS_ROOT/quarkus-workshop-m3-labs/spring-petclinic
----
