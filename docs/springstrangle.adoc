...
Append the following variables in `src/main/resources/application.properties` within the `quarkus-petclinic-customers-service` project:
...
Append the following variables in `src/main/resources/application.properties` within the `quarkus-petclinic-visits-service` project:
...
[source,sh,role="copypaste"]
----
mvn clean package -Duser.home=/home/jboss -Duser.home=/home/jboss -f $CHE_PROJECTS_ROOT/quarkus-workshop-m3-labs/quarkus-petclinic-vets-service
----
