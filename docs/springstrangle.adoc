...
Our production environment needs a "real" database so let's update the Postgres database connection details (JDBC url, database credentials) which are specified in `application.properties`. So you'll update the below details now:
